...
class magic_helper:
    ...
    def plot_model_training_performance(history):
        ...
        print("\n" + "\033[1m" + "*" * 100);
        print(" " * 25 + f"Model Training Plots");
        print("*" * 100 + '\033[0m');
        print()
    ...
    def visualize_ten_misclassified_samples(test_image_generator_x,
        ...
        '''
        fig, axs = plt.subplots(1, 10, sharey=True, figsize=(30, 30))
        for i in range(10):
            axs[i].imshow(np.squeeze(test_image_generator_x[misclass_imgs_indices[i]]), cmap='gray')
            axs[i].set_title(f'True Label: {misclass_imgs_true[i]}\nPredicted Label: {misclass_imgs_pred[i]}',
                             fontsize=19, pad=20)
